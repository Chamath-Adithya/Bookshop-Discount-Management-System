...
public class AdminController {
    ...
    private void handleAddProduct(ActionEvent event) {
        ...
        try {
            ...
            if (editingProduct && editingProductId != null && editingProductId.equals(productId)) {
                ...
                if (existing != null) {
                    ...
                    System.err.println("[AdminController] Product to update not found: " + productId);
                }
            } else {
                // Add new product
    ...
    private void handleAddCustomer(ActionEvent event) {
        try {
            ...
            idDlg.setTitle("Add Customer");
            idDlg.setHeaderText("New Customer ID");
            idDlg.setContentText("ID:");
            idDlg.showAndWait().ifPresent(id -> {
                TextInputDialog nameDlg = new TextInputDialog();
                nameDlg.setTitle("Add Customer");
                nameDlg.setHeaderText("Customer Name");
                nameDlg.setContentText("Name:");
                nameDlg.showAndWait().ifPresent(name -> {
                    ...
                    typeDlg.showAndWait().ifPresent(type -> {
                        ...
                        phoneDlg.setHeaderText("Phone (optional)");
                        phoneDlg.setContentText("Phone:");
                        phoneDlg.showAndWait().ifPresent(phone -> {
                            try {
                                Customer c;
                                if ("VIP".equalsIgnoreCase(type)) c = new VIPCustomer(id, name);
                                else c = new RegularCustomer(id, name);
                                c.setPhone(phone);
                                if (customerService == null) customerService = new CustomerService();
                                customerService.addCustomer(c);
                                loadCustomersData();
                            } catch (Exception ex) {
                                System.err.println("[AdminController] Failed to add customer: " + ex.getMessage());
                            }
                        });
                    });
                });
            ...
            System.err.println("[AdminController] Error in add customer flow: " + e.getMessage());
        }
    }
